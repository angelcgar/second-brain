  ...
  template: `
    ...
    <div class="flex flex-col bg-[#191919] text-white">
      ...
      <main class="flex-1 px-6 pb-6">
        <!-- Responsive Grid layout (minimum 3 columns on large screens) -->
    ...
    @if (selectedArea()) {
      ...
      >
        ...
        >
          <!-- Top metadata row (ID and close button) -->
          ...
          <div class="flex flex-col gap-3 py-3 border-y border-neutral-800/65">
            ...
            <div class="grid grid-cols-[120px_1fr] items-center gap-4">
              ...
              <label class="flex items-center cursor-pointer px-2 py-1">
                <input
                  type="checkbox"
                  [ngModel]="selectedArea()?.archivado"
          ...
          <!-- Bottom actions -->
          <div class="mt-4 pt-3 border-t border-neutral-800/60 flex items-center justify-end">
            <button
              (click)="closeDialog()"
...
export class KanbanBoardAreasComponent implements OnInit {
  ...
  selectedArea = signal<AreaItem | null>(null);
  ...
  // Computed signal for filtered areas
  ...
  // ── Lifecycle ──────────────────────────────────────────────────────
  ngOnInit(): void {
    // Default mock data matching existing dashboard aesthetics
    this.areas.set([
      { id: this.generateId(), title: 'Desarrollo de Software', type: 'Empresa', archivado: false, icon: '💼' },
      { id: this.generateId(), title: 'Finanzas Personales', type: 'Personal', archivado: false, icon: '💰' },
      { id: this.generateId(), title: 'Tesis Doctoral', type: 'Academico', archivado: false, icon: '🎓' },
      { id: this.generateId(), title: 'Gimnasio y Salud', type: 'Personal', archivado: false, icon: '🏋️' },
      { id: this.generateId(), title: 'Investigación IA', type: 'Academico', archivado: false, icon: '🔬' },
      { id: this.generateId(), title: 'Marketing Digital', type: 'Empresa', archivado: false, icon: '📈' },
    ]);
  }
  ...
  addArea(): void {
    ...
    const icons: Record<'Empresa' | 'Personal' | 'Academico', string> = {
      Empresa: '💼',
      Personal: '👤',
      Academico: '🎓'
    };
    
    // Default to 'Personal' for new areas or cycle them
    const types: ('Empresa' | 'Personal' | 'Academico')[] = ['Personal', 'Empresa', 'Academico'];
    const chosenType = types[this.areaCounter % 3];
    ...
    this.areas.set([...currentAreas, newArea]);
  }

  // ── Helpers ────────────────────────────────────────────────────────
  updateField(field: keyof AreaItem, value: any): void {
    const area = this.selectedArea();
    if (area) {
      (area as any)[field] = value;
      this.selectedArea.set({ ...area });

      // Update in signal list
      const currentAreas = this.areas();
      const index = currentAreas.findIndex(a => a.id === area.id);
      if (index !== -1) {
        // Adjust icon if type changed
        if (field === 'type') {
          const icons: Record<'Empresa' | 'Personal' | 'Academico', string> = {
            Empresa: '💼',
            Personal: '👤',
          ...
          area.icon = icons[value as 'Empresa' | 'Personal' | 'Academico'] || '📁';
        }
        currentAreas[index] = { ...area };
