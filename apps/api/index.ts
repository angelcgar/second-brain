...
const allowedTaskStatuses: TaskStatus[] = [
  "inbox",
  "esperando",
  "sin_fecha",
  "en_proceso",
...
const taskSelectAllStmt = db.query(
...
const projectSelectAllStmt = db.query(
  "SELECT * FROM projects WHERE archivo != 1 ORDER BY created_at ASC",
);
...
const areaSelectAllStmt = db.query(
  "SELECT * FROM areas WHERE archivado != 1 ORDER BY title COLLATE NOCASE ASC",
);
...
const goalSelectAllStmt = db.query(
  "SELECT * FROM goals WHERE archivado != 1 ORDER BY created_at ASC",
);
...
function validateTaskStatus(status: unknown): status is TaskStatus {
  return (
    ...
    allowedTaskStatuses.includes(status as TaskStatus)
  );
}
...
function validateAreaType(value: unknown): value is AreaType {
  return (
    typeof value === "string" && allowedAreaTypes.includes(value as AreaType)
  );
}
...
function validateGoalStatus(value: unknown): value is GoalStatus {
  return (
    typeof value === "string" &&
    allowedGoalStatuses.includes(value as GoalStatus)
  );
}
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (req.method === "POST" && pathname === "/tasks") {
      const payload = parsePayload<TaskPayload>(
        await req.json().catch(() => null),
      );
      if (
        !payload ||
        typeof payload.title !== "string" ||
        !validateTaskStatus(payload.status)
      ) {
    ...
    if (req.method === "POST" && pathname === "/projects") {
      const payload = parsePayload<ProjectPayload>(
        await req.json().catch(() => null),
      );
      if (
        !payload ||
      ...
      if (!createdProject)
        return json({ error: "No se pudo crear el proyecto" }, 500);
    ...
    if (req.method === "PUT" && pathname.startsWith("/projects/")) {
      ...
      projectUpdateStmt.run(
        ...
        payload.archivo !== undefined
          ? boolToInt(payload.archivo)
          : (existingProject.archivo ?? 0),
      ...
      if (!updatedProject)
        return json({ error: "No se pudo actualizar el proyecto" }, 500);
    ...
    if (req.method === "POST" && pathname === "/areas") {
      const payload = parsePayload<AreaPayload>(
        await req.json().catch(() => null),
      );
    ...
    if (req.method === "PUT" && pathname.startsWith("/areas/")) {
      ...
      areaUpdateStmt.run(
        ...
        nextType,
        payload.archivado !== undefined
          ? boolToInt(payload.archivado)
          : (existingArea.archivado ?? 0),
        id,
      );

      const updatedArea = getAreaById(id);
      if (!updatedArea)
        return json({ error: "No se pudo actualizar el área" }, 500);
    ...
    if (req.method === "POST" && pathname === "/goals") {
      const payload = parsePayload<GoalPayload>(
        await req.json().catch(() => null),
      );
      if (
        !payload ||
    ...
    if (req.method === "PUT" && pathname.startsWith("/goals/")) {
      ...
      goalUpdateStmt.run(
        ...
        payload.archivado !== undefined
          ? boolToInt(payload.archivado)
          : (existingGoal.archivado ?? 0),
        id,
      );

      const updatedGoal = getGoalById(id);
      if (!updatedGoal)
        return json({ error: "No se pudo actualizar el goal" }, 500);
